...
void* tls_try_alloc(SlabAllocator* a, uint32_t sc, uint32_t epoch_id, SlabHandle* out_h) {
    fprintf(stderr, "[DEBUG] tls_try_alloc: ENTER sc=%u epoch=%u bypass=%d\n", sc, epoch_id, _tls_bypass);
    fflush(stderr);
    
    if (_tls_bypass) {
        fprintf(stderr, "[DEBUG] tls_try_alloc: BYPASS - returning NULL\n");
        fflush(stderr);
        return NULL;  /* Skip TLS during flush */
    }
    
    if (!_tls_initialized) {
        fprintf(stderr, "[DEBUG] tls_try_alloc: initializing thread\n");
        fflush(stderr);
        tls_init_thread();
    }
    
    TLSCache* tls = &_tls_cache[sc];
    fprintf(stderr, "[DEBUG] tls_try_alloc: tls->count=%u\n", tls->count);
    fflush(stderr);
    
    if (tls->count > 0) {
        fprintf(stderr, "[DEBUG] tls_try_alloc: cache has handles, popping\n");
        fflush(stderr);
        ...
        fprintf(stderr, "[DEBUG] tls_try_alloc: idx=%u handle=0x%lx cached_epoch=%u\n", idx, (unsigned long)h, cached_epoch);
        fflush(stderr);
        
        if (cached_epoch == epoch_id) {
            fprintf(stderr, "[DEBUG] tls_try_alloc: epoch MATCH\n");
            fflush(stderr);
            ...
            *out_h = h;
            ...
            fprintf(stderr, "[DEBUG] tls_try_alloc: unpacked slab_id=%u gen=%u slot=%u\n", slab_id, gen, slot);
            fflush(stderr);
            
            Slab* s = reg_lookup_validate(&a->reg, slab_id, gen);
            if (!s) {
                fprintf(stderr, "[DEBUG] tls_try_alloc: VALIDATION FAILED - returning NULL\n");
                fflush(stderr);
                return NULL;
            }
            
            void* ptr = slab_slot_ptr(s, slot);
            fprintf(stderr, "[DEBUG] tls_try_alloc: SUCCESS - returning %p\n", ptr);
            fflush(stderr);
            return ptr;
        } else {
            fprintf(stderr, "[DEBUG] tls_try_alloc: epoch MISMATCH (cached=%u requested=%u) - returning NULL\n", cached_epoch, epoch_id);
            fflush(stderr);
        }
        
    } else {
        fprintf(stderr, "[DEBUG] tls_try_alloc: cache EMPTY - returning NULL\n");
        fflush(stderr);
    }
...
bool tls_try_free(SlabAllocator* a, uint32_t sc, SlabHandle h) {
    fprintf(stderr, "[DEBUG] tls_try_free: ENTER sc=%u handle=0x%lx bypass=%d in_flush=%d\n", sc, (unsigned long)h, _tls_bypass, _tls_in_flush);
    fflush(stderr);
    ...
    TLSCache* tls = &_tls_cache[sc];
    fprintf(stderr, "[DEBUG] tls_try_free: tls->count=%u\n", tls->count);
    fflush(stderr);
    ...
    if (tls->count < TLS_CACHE_SIZE) {
        ...
        Slab* s = reg_lookup_validate(&a->reg, slab_id, gen);
        if (!s) {
            fprintf(stderr, "[DEBUG] tls_try_free: validation failed\n");
            fflush(stderr);
            return false;
        }
        
        fprintf(stderr, "[DEBUG] tls_try_free: cached, count now %u\n", tls->count + 1);
        fflush(stderr);
    ...
    fprintf(stderr, "[DEBUG] tls_try_free: cache FULL, flushing batch\n");
    fflush(stderr);
    _tls_in_flush = true;
    tls_flush_batch(a, sc, TLS_FLUSH_BATCH);
    _tls_in_flush = false;
    fprintf(stderr, "[DEBUG] tls_try_free: flush complete, count now %u\n", tls->count);
    fflush(stderr);
    
    tls->handles[tls->count] = h;
    tls->epoch_id[tls->count] = 0;
    tls->count++;
    fprintf(stderr, "[DEBUG] tls_try_free: cached after flush\n");
    fflush(stderr);
    return true;
}
...
void tls_refill(SlabAllocator* a, uint32_t sc, uint32_t epoch_id) {
    ...
    fprintf(stderr, "[DEBUG] tls_refill: sc=%u epoch=%u count=%u\n", sc, epoch_id, tls->count);
    fflush(stderr);
    ...
    for (uint32_t i = 0; i < TLS_REFILL_BATCH && tls->count < TLS_CACHE_SIZE; i++) {
        fprintf(stderr, "[DEBUG] tls_refill: iteration %u\n", i);
        fflush(stderr);
        SlabHandle h;
        ...
        fprintf(stderr, "[DEBUG] tls_refill: alloc_obj_epoch returned %p\n", ptr);
        fflush(stderr);
        ...
        fprintf(stderr, "[DEBUG] tls_refill: epoch state = %u\n", state);
        fflush(stderr);
        if (state != EPOCH_ACTIVE) {
            fprintf(stderr, "[DEBUG] tls_refill: epoch closed, freeing handle\n");
    ...
    fprintf(stderr, "[DEBUG] tls_refill: done, final count=%u\n", tls->count);
    fflush(stderr);
